...
Nachdem der Rektofizierungsalgorithmus für das synthetische Beispiel anhand von Kameras gleicher Auflösung 

Im Folgenden wird getesetet, welche Ergebnisse der zuvor entwickelte Rektifizierungsalgorithmus mit Bilder von Kameras unterschiedlicher erbringt. Im ersten Beispiel wird für $C$ die Auflösung $\zeta_x = \zeta_y =1$ gewählt. Die Kameramatrix $K$ lautet wie folgt
...
Für $C'$ wurde eine kleinere Auflösung mit $\zeta'_x = \zeta'_y = 0.5$ gewählt. die Kameramatrix $K'$ lautet somit
...
Die entstehenden Bilder des Quaders sind in Abbildung \ref{fig:AbbRecDifRes} zu sehen. Da $K'$ nur eine halb so große Auflösung wie $K$ besitzt, ist das resultierende Bild des roten Quaders auch nur halb so groß. Der güne Quader zeigt das entstehende Bild von $C'$.
...
Die folgenden Abbildungen zeigen die einzelnen Transformationen. Abbildung \ref{fig:AbbRecDifResHp} zeigt die projektive Transformation der beiden Bilder mit $H_p$ und $H_p'$. Die Epipolarlinien der beiden Bilder sind jetzt jeweils parallel zueinander. Abbildung \ref{fig:AbbRecDifResHrHp} zeigt Transformation mit $H_r$ und $H_r'$. Die Epipolarlinien sind jetzt parallel zur horizontalen Achse und die Epipolarlinien von Bild eins und Bild zwei sind auch zueinander ausgerichtet. Hier ist zu beobachten, dass das durch das Ausrichten der Epipolarlinien zueinander das Bild des roten Quaders stark horizontal und vertikal verzerrt wird. 
...
\begin{figure}[!htb]
	...
	\caption[Transformation $H_p$ und $H_p'$ angewandt auf Bilder unterschiedlicher Auflösungen]{}
	\label{fig:AbbRecDifResHp}
	\endminipage\hfill
	\minipage{0.48\textwidth}
	\includegraphics[width=\linewidth]{images/Rectification_HrHp_different_Solutions.png}
	\caption[Transformation $H_rH_p$ und $H_r'H_p'$ angewandt auf Bilder unterschiedlicher Auflösungen]{}
...
\begin{figure}[!htb]
	...
	\caption[Transformation $H_sH_rH_p$ und $H_s'H_r'H_p'$ angewandt auf Bilder unterschiedlicher Auflösungen]{}
	...
	\caption[Transformation $H_sH_rH_p$ und $H_s'H_r'H_p'$ angewandt auf Bilder unterschiedlicher Auflösungen mit Epipolarlinien]{}
...
Für $K$ von $C$ gilt weiterhin $\zeta_x = \zeta_y = 1$. Die Entstehenden Bilder sind in Abbildung \ref{fig:7.18} zu sehen. Die horizontale Kantenlänge des Bildes in $C'$ mit dem roten Quader ist im Verhältnis kürzer als ihre vertikale Kantenlänge. Dieses Verhältnis wird nach der rektifizierung weiterhin beibehalten, weshalb der Horizontale Versatz der horizontalen Koordinaten in beiden Bilder nicht stimmt, wie in Abbildung \ref{fig:7.19} zu sehen ist. Dies könnte bei der Erstellung der Tiefenkarte Probleme dazu führen, dass die Tiefen falsch berechnet werden und es somit zu einer Fehlerhaften Rekonstruktion der 3D-Szene kommt.
