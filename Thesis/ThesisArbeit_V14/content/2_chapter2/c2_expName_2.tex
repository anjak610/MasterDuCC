...
Die mit dem \textit{SURF}-Algorithmus gefundenen Punkte sind mit den gelben Ziffern in den Abbildungen \ref{fig:SurfLinks} und \ref{fig:SurfRechts} gekennzeichnet. Abbildung \ref{fig:SurfLinks} zeigt das Bild von $C$, die Abbildung \ref{fig:SurfRechts} zeigt das Bild von $C'$.\\
...
%Angenommen die korrespondierenden Koordinaten behalten ihre originalen Pixelwerte, dann  
...
%Die Normierung der Koordinaten führt dazu, dass die Größenordnungen der Einträge der Fundamentalmatrix ausgeglichener sind. Wird angenommen, dass die Koordinaten ihre ursprünglichen Größeneinheiten behalten, so haben sie beispielsweise die Form
%
%\begin{gather}
...
%\end{gather}.
%
%Eine Fundamentalmatrix basierend auf diesen Koordinatenverhältnissen, hätte starke Größenunterschiede in ihren Einträgen
%
%\begin{gather}
%	F = \begin{pmatrix}
%		10^{-8}&10^{-6}&10^{-4}\\
...
%\end{gather}.
%
%Sind jedoch die Koordinaten in einer Form, bei welcher die drei Koordinateneinträge sich in einer ähnlichen Größenordnung befinden wie beispielsweise
%
% \begin{gather}
% 	m\sigma = \begin{pmatrix}
% 		1.5\\1.193\\1
...
% \end{gather}
%
%so wären die Einträge der Fundamentalmatrix in ihren Größenordnungen ausgeglichener\cite{HZ8}
%
%\begin{gather}
%	F = \begin{pmatrix}
%		10^{-4}&10^{-4}&10^{-2}\\
...
%\end{gather}
% 
%Hier schreiben was die Augeglicheneheit genau bewirkt\\
%
%The most important entries in the fundamental
%matrix are precisely those that are subject
%to the largest relative perturbation when
%enforcing the singularity constraint without
%prior normalization.
%
%The method described above of enforcing the singularity
%constraint gives the singular matrix ˆ F nearest to
%F in Frobenius norm.The trouble with this method is
%that it treats all entries of the matrix equally, regardless
%of their magnitude.T hus, entries of F small in
%absolute value may be expected to undergo a perturbation
%much greater relative to their magnitude than
%the large entries.
